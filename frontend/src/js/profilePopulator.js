...
export class ProfilePopulatorService {
            ...
            mutations.forEach((mutation) => {
                if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                    ...
                    if (appArea && !appArea.classList.contains('hidden')) {
                        console.log('App area became visible, populating height/age...');
    ...
    async populateHeightAndAgeFromProfile() {
        console.log('Auto-populating height and age from profile...');
        ...
        try {
            ...
            if (!token) {
                console.log('No auth token, skipping auto-population');
            ...
            if (userActive) {
                ...
                console.log('Using UserActive endpoint for profile population:', endpoint);
            } else {
                console.log('Using authenticated user endpoint for profile population:', endpoint);
            }
            ...
            if (response.ok) {
                ...
                console.log('User profile loaded for population:', user);
                ...
                console.log('Failed to fetch user profile for population');
                if (heightStatus) heightStatus.classList.add('hidden');
            }
        } catch (error) {
            console.log('Error auto-populating height/age:', error);
            if (heightStatus) heightStatus.classList.add('hidden');
        }
    }
    ...
    populateHeightField(user, heightStatus) {
        ...
        if (heightInput && user && user.height) {
            ...
            console.log('Height auto-populated:', user.height);
    ...
    populateAgeField(user) {
        ...
        if (ageInput && user && user.birthday) {
            ...
            if (age !== null) {
                ...
                console.log('Age auto-calculated:', age);
            }
        } else if (ageInput) {
