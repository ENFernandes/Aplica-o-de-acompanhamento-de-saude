-- Health Tracker Database Update Script
-- This script adds new features and improvements to the existing database
...
-- Create a table for exercise tracking
...
-- Create indexes for exercise records
...
-- Create a table for nutrition tracking
...
-- Create indexes for nutrition records
...
-- Create a table for measurements tracking (circumference measurements)
...
-- Create indexes for measurement records
...
-- Create a table for app settings and preferences
...
-- Add some sample goals for the test user
DO $$
DECLARE
    test_user_id UUID;
BEGIN
    ...
    IF test_user_id IS NOT NULL THEN
        -- Insert sample goals
        INSERT INTO user_goals (user_id, goal_type, target_value, current_value, start_date, target_date) VALUES
            (test_user_id, 'weight', 85.0, 87.9, '2025-07-24', '2025-12-31'),
            (test_user_id, 'body_fat', 18.0, 21.6, '2025-07-24', '2025-12-31'),
            (test_user_id, 'muscle_mass', 70.0, 65.7, '2025-07-24', '2025-12-31')
        ON CONFLICT DO NOTHING;
        
        -- Insert sample exercise records
        INSERT INTO exercise_records (user_id, date, exercise_type, duration_minutes, calories_burned, intensity) VALUES
            (test_user_id, '2025-07-24', 'Weight Training', 60, 400, 'high'),
            (test_user_id, '2025-07-25', 'Cardio', 30, 300, 'medium'),
            (test_user_id, '2025-07-26', 'Yoga', 45, 200, 'low')
        ON CONFLICT DO NOTHING;
        
        -- Insert sample nutrition records
        INSERT INTO nutrition_records (user_id, date, meal_type, food_name, calories, protein_grams, carbs_grams, fat_grams) VALUES
            (test_user_id, '2025-07-24', 'breakfast', 'Oatmeal with berries', 350, 12, 60, 8),
            (test_user_id, '2025-07-24', 'lunch', 'Grilled chicken salad', 450, 35, 15, 25),
            (test_user_id, '2025-07-24', 'dinner', 'Salmon with vegetables', 550, 40, 20, 30)
        ON CONFLICT DO NOTHING;
        
        -- Insert sample measurement records
        INSERT INTO measurement_records (user_id, date, chest_cm, waist_cm, hips_cm, biceps_cm, thighs_cm) VALUES
            (test_user_id, '2025-07-24', 105.5, 85.0, 95.0, 35.0, 58.0)
        ON CONFLICT DO NOTHING;
        
        -- Insert sample user settings
        INSERT INTO user_settings (user_id, setting_key, setting_value) VALUES
            (test_user_id, 'units', 'metric'),
            (test_user_id, 'theme', 'light'),
            (test_user_id, 'notifications', 'true'),
            (test_user_id, 'goal_reminders', 'true')
        ON CONFLICT DO NOTHING;
    END IF;
END $$;

-- Add sample goals for admin user
DO $$
DECLARE
    admin_user_id UUID;
BEGIN
    ...
    IF admin_user_id IS NOT NULL THEN
        -- Insert sample goals for admin
        INSERT INTO user_goals (user_id, goal_type, target_value, current_value, start_date, target_date) VALUES
            (admin_user_id, 'weight', 80.0, NULL, '2025-07-28', '2025-12-31'),
            (admin_user_id, 'body_fat', 15.0, NULL, '2025-07-28', '2025-12-31')
        ON CONFLICT DO NOTHING;
        
        -- Insert sample user settings for admin
        INSERT INTO user_settings (user_id, setting_key, setting_value) VALUES
            (admin_user_id, 'units', 'metric'),
            (admin_user_id, 'theme', 'dark'),
            (admin_user_id, 'notifications', 'true'),
            (admin_user_id, 'goal_reminders', 'true')
        ON CONFLICT DO NOTHING;
    END IF;
END $$;

-- Create a view for comprehensive health summary
...
-- Create a function to calculate BMI automatically
...
CREATE OR REPLACE FUNCTION get_user_progress(p_user_id UUID, p_days INTEGER DEFAULT 30)
RETURNS TABLE (
    date DATE,
    weight DECIMAL(5,2),
...
-- Update existing health records to calculate BMI if missing
